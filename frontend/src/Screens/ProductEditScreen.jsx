...
const ProductEditScreen = () => {
  ...
  const {loading:updateLoading,success:updateSuccess,error:updateError}=useSelector((state)=>state.productUpdate)
  ...
  async function productUpdater(){
    try {
        ...
        const {data}=await axios.put(`/api/products/${id}`,{id,name,price,image,brand,category,countInStock,description},config)
        dispatch(productUpdateAction.updateSuccess(data))
    } catch (error) {
        dispatch(productUpdateAction.updateFail(error.response && error.response.data.message ? error.response.data.message : error.message))
    }
  }
